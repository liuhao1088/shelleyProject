...
Page({
  ...
  onShow: function () {
    ...
    switch(wx.getStorageSync('userInfo')!==''){
      case true:
        wx.cloud.database().collection('message').where({_openid:app.globalData.openid,read:'unread'}).count({
        ...
        if(!wx.getStorageSync('prize')){
          wx.cloud.database().collection('coupon').where({_openid:app.globalData.openid,shop_code:'all'}).get().then(res=>{
            let data=res.data;
            if(data.length==0){
              _t.setData({prize:true})
            }else{
