...
Page({
   ...
   */
  data: {
    list:[],search:'',searchlist:[],search_whether:false
  },
  ...
  toSearch:function(){
    if(this.data.search!==''){
      ...
      wx.cloud.database().collection('reservation').where({shop_code:userInfo.shop[userInfo.shop.length-1].shop_code}).where({re_code:{
        ...
        if(res.data.length==0){
          ...
          that.setData({search_whether:true})
        }
      })
    }else{
  ...
  loadData:function(){
    ...
    let userInfo=wx.getStorageSync('userInfo')
    wx.cloud.database().collection('reservation').where({shop_code:userInfo.shop[userInfo.shop.length-1].shop_code}).orderBy('creation_date','desc').skip(skip).limit(5).get().then(res=>{
      let data=res.data;
      for(let i=0;i<data.length;i++){
        data[i].label=data[i].shopping.join(',')
        if(i+1==data.length){
        ...
        if (JSON.stringify(res).indexOf('accept') !== -1) {
          wx.cloud.callFunction({
            ...
            data:{
              ...
              where:{_id:that.data.list[ind]._id},
              updateData:{
                status:'success'
              }
            }
